...
public class A1main {
    ...
    /**
     * Adds valid wind directions affecting the cost of the path
     * Done for parameter validation
     * @param windDirections the list to which the valid wind directions will be added
     */
    ...
    public static void printFrontier(Frontier frontier) {
        System.out.println("Frontier has: " + frontier.getFrontier().size() +" nodes.");
    ...
    public static void printExpandedNodes() {
        System.out.println("Expanded: " + expandedNodes.size() + " nodes:");
    ...
    public static Node breadthFirstSearch() {
        ...
        while (true) {
            ...
            System.out.println("Current node to be expanded: " + currentNode.toString());
            System.out.println();
    ...
    public static Node bestFirstSearch() {
        ...
        while (true) {
            ...
            if (isGoal(currentNode.getState())) {
                ...
                for (Node possibleMove : getPossibleMoves(currentNode)) {
                    if (!expandedNodes.contains(possibleMove) && !frontier.getFrontier().contains(possibleMove)) {
                        frontier.getFrontier().add(possibleMove);
                    }
                }
            }
    ...
    public static Node bidirectionalSearch() {
        ...
        while (true) {
            ...
            printFrontier(frontier);
            System.out.println("'Reverse' frontier: ");
            printFrontier(reverseFrontier);
            printExpandedNodes();
            // Get first item on frontier i.e. shallowest node
            Node currentNode = frontier.getFrontier().pollFirst();
            expandedNodes.add(currentNode);
            ...
            System.out.println("Current 'reverse' node to be expanded: " + currentReverseNode.toString());
            System.out.println();
            exploredStates.add(currentReverseNode.getState());
            expandedNodes.add(currentReverseNode);
            printExpandedNodes();
            for (Node possibleMove: getPossibleMoves(currentNode)) {
                Node childNode = possibleMove;
                if (!exploredStates.contains(childNode) && !frontier.getFrontier().contains(childNode)){
    ...
    public static void printPath(Node node) {
        ...
        System.out.println("Goal found: " + node.getState().toString());
        System.out.println("Expanded: " + expandedNodes.size() + " nodes");
        System.out.println("Path has cost: " + String.format("%.02f", node.getPathCost()));
        ...
        System.out.println("Path has: " + (stack.size()-1) + " steps");
    ...
    public static void main(String args[]) throws InvalidArgumentsException, ImpossibleFlightException {
        ...
        if (args.length != 4 && args.length != 5) {
            ...
            try {
                ...
                if (finalNode == null) {
                    throw new ImpossibleFlightException("Impossible flight: Can't find path");
                }
                out.println(searchAlgorithm + "," + finalNode.getPathCost() + "," + expandedNodes.size());
                out.flush();
                out.close();
                printPath(finalNode);
            }
                ...
                System.out.println("Usage: <DFS|BFS|AStar|BestF>|<BiDir> <N> <d_s,angle_s> <d_g,angle_g> <wind_direction>");
            }
            catch(IOException e) {
                System.out.println("Exception: " + e.getMessage());
            }
